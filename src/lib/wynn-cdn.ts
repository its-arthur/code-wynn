...
const WYNN_NEXTGEN_BASE =
  process.env.NEXT_PUBLIC_WYNN_CDN_NEXTGEN ?? "https://cdn.wynncraft.com/nextgen";
...
export function wynnClassPictureUrl(className: string, ext: "webp" | "png" = "webp"): string {
  return wynnNextgenUrl("classes", "picture", `${className.toLowerCase()}.${ext}`);
}

/** Leaderboard / profession / skill list icon e.g. fishing.webp, strength.webp */
export function wynnLeaderboardIconUrl(name: string, ext: "webp" | "png" | "svg" = "webp"): string {
...
export function wynnSkillIconUrl(filename: string): string {
  const name = filename.endsWith(".svg") || filename.endsWith(".webp") || filename.endsWith(".png")
    ? filename
    : `${filename}.webp`;
...
export function wynnFontUrl(fontName: string, ext: "woff" | "woff2" = "woff"): string {
...
export function wynnDungeonIconUrl(name: string, ext: "webp" | "png" = "webp"): string {
  const slug = (name ?? "").trim().replace(/\s+/g, "_").replace(/-/g, "_").toLowerCase();
...
export function wynnRaidIconUrl(name: string, ext: "webp" | "png" = "webp"): string {
