...
const SKILL_ENTRIES: {
...
}[] = [
	{
		key: "strength",
		label: "STR",
		iconName: "strength_book.svg",
		textClass: "text-green-600 dark:text-green-400",
	},
	{
		key: "dexterity",
		label: "DEX",
		iconName: "dexterity_book.svg",
		textClass: "text-yellow-600 dark:text-yellow-400",
	},
	{
		key: "intelligence",
		label: "INT",
		iconName: "intelligence_book.svg",
		textClass: "text-blue-600 dark:text-blue-400",
	},
	{
		key: "defense",
		label: "DEF",
		iconName: "defense_book.svg",
		textClass: "text-red-600 dark:text-red-400",
	},
	{
		key: "agility",
		label: "AGI",
		iconName: "agility_book.svg",
						...
						<CardContent className="gap-4 flex flex-col px-4 py-3">
							<div className="w-full items-center gap-2 grid grid-cols-[30%_70%]">
								...
								<div className="flex text-xs min-w-0 flex-col justify-center gap-0.5">
									<span
										className=" font-medium text-muted-foreground capitalize font-minecraft truncate"
										title={name}
									>
										{name}
									</span>
										...
										<>
											...
											<div className="flex min-w-0 flex-1 flex-col gap-0.5">
												<span className="leading-tight">
													{selectedChar.type}
													{selectedChar.reskin != null &&
														selectedChar.reskin !== selectedChar.type && (
												...
												<span className="text-sm text-muted-foreground leading-tight">
													...
													{selectedChar.playtime != null &&
														` · ${formatPlaytime(selectedChar.playtime)} playtime`}
												</span>
											</div>
											<SelectValue
						...
						<>
							<div className="flex w-full flex-col gap-4 lg:flex-row lg:items-stretch">
								...
								<div className="min-w-0 w-full flex-1">
									{selectedChar && <CharacterFullCard char={selectedChar} />}
								</div>
							</div>
							...
							<section>
								<Collapsible defaultOpen className="group">
									...
									<CollapsibleContent>
										{Object.keys(selectedChar.dungeons?.list ?? {}).length ===
										0 ? (
											<p className="py-8 text-center text-sm text-muted-foreground">
											...
											<div className="grid w-full grid-cols-2 gap-4 sm:grid-cols-4">
												{Object.entries(selectedChar.dungeons?.list ?? {}).map(
													([name, count]) => (
														<Card key={name} className="min-w-0">
															<CardContent className="grid grid-cols-[30%_70%] gap-x-3 gap-y-1 px-4 py-3">
																...
																<div className="flex min-w-0 flex-col justify-center gap-0.5">
																	<span
																		className="text-xs font-medium text-muted-foreground font-minecraft truncate"
																		title={name}
																	>
																		{name}
																	</span>
							...
							<section>
								<Collapsible defaultOpen className="group">
									...
									<CollapsibleContent>
										{Object.keys(selectedChar.raids?.list ?? {}).length ===
										0 ? (
											<p className="py-8 text-center text-sm text-muted-foreground">
												No raids data found.
											</p>
											...
											<div className="grid w-full grid-cols-2 gap-4 sm:grid-cols-4">
												{Object.entries(selectedChar.raids?.list ?? {}).map(
													([name, count]) => (
														<Card key={name} className="min-w-0">
															<CardContent className="grid grid-cols-[30%_70%] gap-x-3 gap-y-1 px-4 py-3">
																<Image
							...
							<section>
								...
								<div className="flex w-full flex-col sm:flex-row flex-wrap gap-4 overflow-x-auto">
									{SKILL_ENTRIES.map((e) => (
										<Card key={e.key} className={`min-w-20 flex-1 shrink-0`}>
											<CardContent className="flex flex-col items-center gap-2 p-4">
												...
												/>
												<span
													className={`text-xs font-medium text-muted-foreground capitalize font-minecraft ${e.textClass}`}
												>
													{e.label}
												</span>
												<span
													className={`text-2xl font-pixel-square tabular-nums`}
												>
