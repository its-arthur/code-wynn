...
function getAbilityNodeSizePx(
	icon: AbilityMapAbilityEntry["meta"]["icon"],
): number {
...
export function AbilityMapGrid({
	...
	const abilities = entries.filter(
		(e): e is AbilityMapAbilityEntry => e.type === "ability",
	);
	const connectors = entries.filter(
		(e): e is AbilityMapConnectorEntry => e.type === "connector",
	);
	...
	return (
		...
		>
			<div className="relative mx-auto w-full" style={{ width: cellPx * cols }}>
				<div
				...
				>
					{abilities.map((e, i) => {
						...
						const defaultIconUrl = getAbilityNodeIconUrl(
							e.meta.icon,
							"default",
						);
						...
						const trigger = (
							<div className="group/trigger relative flex flex-col items-center justify-center w-full h-full min-w-0 min-h-0 rounded-lg transition-opacity duration-200 hover:opacity-90 hover:cursor-pointer">
								<div
									className="pointer-events-none absolute inset-[-6px] z-0 rounded-lg opacity-0 transition-opacity duration-200 group-hover/trigger:opacity-100"
									style={{
										background: showActiveIcon
											? "radial-gradient(circle, rgba(78, 146, 255, 0.45) 0%, transparent 80%)"
						...
						const wrappedCell = showAbilityPopover ? (
							...
							>
								<Popover open={hoveredAbilityId === e.meta.id}>
					...
					{connectors.map((e, i) => {
						...
						const activeIconName =
							activeConnectorIconNames.get(connectorKey) ?? defaultIconName;
